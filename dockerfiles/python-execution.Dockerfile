...
RUN chmod +x ./gradlew \
    ...
    && mv ./python-execution-service/build/libs/python-execution-service-1.0.0.jar ./python-execution-service.jar \
    && apt update \
    && apt install unzip -y \
    && unzip ./python-execution-service.jar -d temp
...
RUN $JAVA_HOME/bin/jdeps \
      ...
      ./python-execution-service.jar > ./jre-modules.txt \
    && $JAVA_HOME/bin/jlink \
      --verbose \
