...
RUN chmod +x ./gradlew \
    && ./gradlew :c-execution-service:build -x test \
    && mv ./c-execution-service/build/libs/c-execution-service-1.0.0.jar ./c-execution-service.jar \
    && apt update \
    && apt install unzip -y \
    && unzip ./c-execution-service.jar -d temp
...
RUN $JAVA_HOME/bin/jdeps \
      ...
      ./c-execution-service.jar > ./jre-modules.txt \
    && $JAVA_HOME/bin/jlink \
      --verbose \
...
COPY --from=jre-build /app/codehorn/c-execution-service.jar /app/c-execution-service.jar
...
CMD ["java", "-Dserver.port=80", "-jar", "/app/c-execution-service.jar"]