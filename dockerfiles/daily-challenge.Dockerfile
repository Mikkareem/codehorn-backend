...
RUN chmod +x ./gradlew \
    ...
    && mv ./daily-challenge-service/build/libs/daily-challenge-service-1.0.0.jar ./daily-challenge-service.jar \
    && apt update \
    && apt install unzip -y \
    && unzip ./daily-challenge-service.jar -d temp
...
RUN $JAVA_HOME/bin/jdeps \
      ...
      ./daily-challenge-service.jar > ./jre-modules.txt \
    && $JAVA_HOME/bin/jlink \
      --verbose \
